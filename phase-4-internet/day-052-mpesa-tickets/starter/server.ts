...
export function createPaymentServer(options: PaymentServerOptions): { server: Server; orders: Map<string, Order> } {
  ...
  async function callback(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    // TODO: parseCallback(await readJson(request)). If that throws: log it, and accept.
    // TODO: find the order by its CheckoutRequestID (byCheckout). Unknown -> log and accept.
    // TODO: an order that isn't "pending" any more -> log "repeated callback" and accept (the first one decides)
    // TODO: paid: if the amount doesn't match totalKes, mark it "failed" with a note for a person to check.
    //   Otherwise "paid", keep the receipt, note "Paid. M-Pesa receipt <receipt>. Enjoy the show!"
    // TODO: cancelled / failed: that status, with the reason as the note. Then accept.
    void parseCallback;
    return accepted();
  }
