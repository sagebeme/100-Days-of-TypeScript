...
export function resendSender(apiKey: string, from: string, fetchFn: Fetcher): EmailSender {
  // TODO: return { send }. send(email) POSTs to "https://api.resend.com/emails" with
  //   headers: Authorization: `Bearer ${apiKey}`, Content-Type: application/json
  //   body (JSON): { from, to: [email.to], subject, text, html }
  //   If the response isn't ok, throw `Email failed (<status>): <the response's text>`
