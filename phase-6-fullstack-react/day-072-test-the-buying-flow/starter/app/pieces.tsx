...
// hear the new number: role="status" is a polite live region, which waits until they've finished
...
export function SeatsLeft({ event }: { event: Pick<TikitiEvent, "status" | "available" | "capacity"> }) {
  ...
  return (
    <span className="badge" data-tone={tone} data-flash={flash || undefined} role="status">
      {label}
    </span>
  );
}
