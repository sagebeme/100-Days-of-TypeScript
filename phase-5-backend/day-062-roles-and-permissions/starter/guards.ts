...
export const requireUser = createMiddleware<SignedInEnv>(async (c, next) => {
  // TODO: no user -> 401 "Log in first"; otherwise carry on
  await next();
});
...
export function requireRole(...roles: Role[]) {
  return createMiddleware<SignedInEnv>(async (c, next) => {
    // TODO: no user -> 401 "Log in first"
    // TODO: a role not in `roles` -> 403 `Only <roles joined with " or "> accounts can do that`
    void HTTPException;
    void roles;
    await next();
  });
