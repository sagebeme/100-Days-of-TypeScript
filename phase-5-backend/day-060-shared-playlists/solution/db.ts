...
export interface Database {
  ...
  close(): void;
  // Runs work inside one transaction: every change commits together, or none of them do.
...
export function openDatabase(path: string): Database {
  ...
  // One connection, one transaction at a time: a simple queue so two transactions never interleave.
  ...
  return { sqlite, db, close: () => sqlite.close(), transaction };
}
