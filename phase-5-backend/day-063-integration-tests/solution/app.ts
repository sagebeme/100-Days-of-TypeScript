...
import { z } from "zod";
import { events, going, users, ROLES, type Role } from "./schema.ts";
...
export interface AppOptions {
  db: Db;
  now?: () => Date;
  // For experiments (Day 63 swaps in broken versions of these to check the tests notice):
  policy?: typeof can;
  creatorRoles?: Role[];
...
export function createApp(options: AppOptions) {
  ...
  const policy = options.policy ?? can;
  const creatorRoles = options.creatorRoles ?? ["organiser", "admin"];
  const protectLastAdmin = options.protectLastAdmin ?? true;
  ...
  function check(user: Parameters<typeof can>[0], action: Action, hideAs404 = false) {
    if (policy(user, action)) return;
    // Someone who can't even see a draft shouldn't learn it exists: that's a 404, not a 403.
    if (hideAs404) throw new HTTPException(404, { message: "No such event" });
    if (!user) throw new HTTPException(401, { message: "Log in first" });
    throw new HTTPException(403, { message: "You can't do that to this event" });
  }
  ...
  app.post("/events", requireRole(...creatorRoles), async (c) => {
  ...
  app.put("/admin/users/:id/role", requireRole("admin"), async (c) => {
    ...
    if (protectLastAdmin && target.role === "admin" && role !== "admin") {
      ...
      if (admins <= 1) throw new HTTPException(409, { message: "That's the last admin. Make someone else an admin first." });
    }
